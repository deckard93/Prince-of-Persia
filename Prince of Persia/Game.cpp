        ...
        input(in) 
{
    ...
    audio = new Audio();
    audio->RegisterSound(Audio::ugh);
    audio->RegisterSound(Audio::drop);
    audio->RegisterSound(Audio::step);
    audio->RegisterSound(Audio::ledge);
    audio->RegisterSound(Audio::potion);
    audio->RegisterSound(Audio::spikes);
    audio->RegisterSound(Audio::gate_up);
    audio->RegisterSound(Audio::trigger);
    audio->RegisterSound(Audio::gate_down);
    audio->RegisterSound(Audio::get_sword);
    audio->RegisterSound(Audio::draw_sword);
    audio->RegisterSound(Audio::gate_close);
    audio->RegisterSound(Audio::spike_kill);
    audio->RegisterSound(Audio::platform_break);
    audio->RegisterSound(Audio::sword_hit_prince);
    audio->RegisterSound(Audio::sword_hit_enemy);
    audio->RegisterSound(Audio::sword_miss);
    audio->RegisterSound(Audio::sword_parry);
    audio->RegisterSound(Audio::title);
    audio->RegisterSound(Audio::intro);
    audio->RegisterSound(Audio::outro);
    audio->RegisterSound(Audio::door_open);


    prince = new Prince(audio);
    
    collision = new Collision(audio, level, &graphics, input);
    ...
    titleMusic = audio->PlaySound(Audio::title);
...
void Game::HandleInput()  
{
    ...
    if (input->getKeyStatus('Y')) {
        titleMusic->Stop();
    }

    if (input->getKeyStatus('T')) {
        titleMusic->Stop();
        titleMusic = audio->PlaySound(Audio::title);
        titleMusic->Start();
    }
    ...
    if(input->isShiftPressed()) {
        ...
        if (level->getSceneCodeByCoord(prince->getMidX(), prince->getMidY()) == '|') {
            if (prince->PickUpSword()) {
                level->setCodeByCoord(prince->getMidX(), prince->getMidY(), '_');
                audio->PlaySound(Audio::get_sword);
            }
        }
    }
    ...
    if (input->isUpPressed()) {
        ...
        if (level->getSceneCodeByCoord(prince->getMidX(), prince->getMidY()) == 'H') {
            ...
            if (e->getType() == doorT) {
                ...
                if (d->Finish() && gameState != gLevelComplete) {
                    prince->setState(sFinish);
                    gameState = gLevelComplete;
                }
...
void Game::Reset() 
{
    level->loadLevel(1);
    prince = new Prince(audio);
    prince->setX(level->getPrinceInitSceneX() * Level::BLOCK_WIDTH_PX - 70);
    prince->setY(level->getPrinceInitSceneY() * Level::BLOCK_HEIGHT_PX + prince->getAnim()->getSheet()->getFrameHeight() - Level::FOOT_FLOAT - Level::BLOCK_HEIGHT_PX + 12); // Magic Number should be size of ledge?
...
void Game::GameLoop() 
{
    ...
    if (gameState == gTitle) {
        DrawTitle();
        HandleInput();
    } else if (gameState == gPause) {
        DrawGraphics();
        HandleInput();
    }
    else if (gameState == gLevelComplete) {
        DrawGraphics();
        HandleInput();
    }
    else if (gameState == gGame) {
        DrawGraphics();
        HandleInput();
...
void Game::DrawGraphics() 
{
    ...
    for (std::map<std::pair<int, int>, Entity*>::iterator i = entitites->begin(); i != entitites->end(); i++) {
        ...
        FinishDoor* finishDoor = dynamic_cast<FinishDoor*>(i->second);
        if (finishDoor != NULL) {
            if (level->inScene(i->first.second, i->first.first)) {
                finishDoor->Animate(&graphics, audio);

                //TODO: move this to the finishDoor object
                if (finishDoor->getAnim() != NULL && finishDoor->getAnim()->isEffectPending() && finishDoor->getAnim()->getCurrentFrame() == 7) {
                    audio->PlaySound(Audio::outro);
                    finishDoor->getAnim()->setEffectDone();
                }
            }
            continue;
        }

        Gate* gate = dynamic_cast<Gate*>(i->second);        
        if (gate != NULL) {
            if (level->inScene(i->first.second, i->first.first)) {
...
void Game::DrawTitle()
{
    graphics.DrawSprite(0, 0, getSprite("title"));
}
        ...
        for (int x = 0; x <= Level::SCENE_WIDTH_BLK; x++) {
            ...
            case '~': {
                ...
                if (level->getSceneBlockXByCoord(prince->getMidX() - princeXOffset) == x &&
                    ...
                    p->Drop();
                    if (p->getAnim()->isEffectPending() && p->getAnim()->getCurrentFrame()== 0) {
                        audio->PlaySound(Audio::ledge);
                        p->getAnim()->setEffectDone();
                    }
...
void Game::CheckCombatCollision() 
{
    ...
    for (std::list<Character*>::iterator i = guards->begin(); i != guards->end(); i++) {
        Character* guard = *i;
        if (guard->isDead()) { continue; }
        if (guard->checkParryBy(prince)) {
            audio->PlaySound(Audio::sword_parry);
        }
        if (guard->isHitting(prince, *level)) {
            audio->PlaySound(Audio::sword_hit_prince);
            prince->Hurt();
        }
    }
    ...
    for (std::list<Character*>::iterator i = guards->begin(); i != guards->end(); i++) {
        ...
        if (prince->isHitting(guard, *level)) {
            audio->PlaySound(Audio::sword_hit_enemy);
            guard->Hurt();
        }
    }
    if (engagedGuard == NULL) { return; }
    ...
    for (std::map<std::pair<int, int>, Entity*>::iterator i = entities->begin(); i != entities->end(); i++) {
        ...
        if (p != NULL && p->getState() == dislodged) {
            ...
            if (level->getLevelCodeByBlock(currentLevelBlockX, currentLevelBlockY) == '_') {
                ...
                audio->PlaySound(Audio::platform_break); //TODO is this the best place for this?
            }
            else {
